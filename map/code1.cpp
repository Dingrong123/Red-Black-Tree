...
void tester(void) {
	...
	//std::cout << Integer::counter<<std::endl<<std::endl;
	...
	//std::cout << Integer::counter<<'e'<<std::endl<<std::endl;
	//	test: erase()
	while (map.begin() != map.end()) {
		map.erase(map.begin());
	}
	//std::cout << Integer::counter<<'f'<<std::endl<<std::endl;
	assert(map.empty() && map.size() == 0);
	//	test: operator[]
	for (int i = 0; i < 100000; ++i) {
		std::cout << map[Integer(i)];
	}
	//std::cout << Integer::counter<<'g'<<std::endl<<std::endl;
